...
export function StickerPicker({ onSelectSticker, onClose, onOpenDrawModal }: StickerPickerProps) {
  ...
  return (
    <div
      ...
      className="bg-[#111b21] text-zinc-100 border border-[#222e35] rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.85)] w-84 sm:w-[410px] max-w-[95vw] h-[520px] flex flex-col overflow-hidden select-none animate-in fade-in zoom-in-95 duration-150 z-50 font-sans"
    >
      ...
      <div className="p-3 pb-2 flex items-center gap-2 border-b border-[#202c33]">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-[#202c33] text-xs sm:text-sm text-white placeholder-zinc-400 rounded-xl pl-9 pr-8 py-2 border border-transparent focus:border-[#00a884] focus:outline-hidden transition"
          />
          {searchQuery && (
            <button
              type="button"
      ...
      <div className="flex items-center justify-center pt-2 pb-1 bg-[#111b21]">
        <div className="flex items-center bg-[#202c33] p-0.5 rounded-full border border-white/5 shadow-inner">
          <button
            type="button"
      ...
      <div className="flex items-center gap-1.5 px-3 py-1.5 overflow-x-auto no-scrollbar scroll-smooth shrink-0 border-b border-[#202c33]">
      ...
      {/* Main Content Area */}
      <div className="flex-1 overflow-y-auto min-h-0 bg-[#0c1317]/60">
        ...
        {activeMode === 'stickers' && (
          <div className="grid grid-cols-4 gap-2.5 p-3">
            ...
            <div
              ...
              className="aspect-square rounded-2xl bg-[#202c33]/90 hover:bg-[#2a3942] border border-white/10 flex flex-col items-center justify-center p-2 text-center transition cursor-pointer group active:scale-95 shadow-sm"
              title="Create custom sticker from photo"
            >
              <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center mb-1 group-hover:bg-[#00a884]/20 transition">
                <ImagePlus className="w-4 h-4 text-zinc-300 group-hover:text-[#00a884]" />
              </div>
              <span className="text-[10px] sm:text-[11px] font-bold text-zinc-200 leading-tight">
                Create sticker
              </span>
            </div>
            ...
            {filteredStickers.map((sticker) => (
              <div
                ...
                className="aspect-square flex items-center justify-center p-1 rounded-2xl hover:bg-white/5 transition duration-150 cursor-pointer group select-none active:scale-90 relative"
                title={sticker.name}
              >
            ...
            {filteredStickers.length === 0 && (
              <div className="col-span-4 py-12 text-center text-zinc-400">
                ...
                <button
                  ...
                  className="mt-2 text-xs font-bold text-[#00a884] hover:underline"
                >
                  View All Stickers
                </button>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 p-3">
            {filteredGifs.map((gif) => (
              <div
                ...
                className="aspect-4/3 rounded-xl overflow-hidden bg-[#202c33] border border-white/5 hover:border-[#00a884] transition cursor-pointer relative group"
                title={gif.name}
              >
                <img
                  src={gif.url}
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-x-0 bottom-0 p-1 bg-gradient-to-t from-black/80 to-transparent text-[10px] font-semibold text-white/90 truncate">
                  {gif.name}
                </div>
              </div>
            ))}
        ...
        {activeMode === 'emoji' && (
          <div className="p-3 space-y-3">
            {EMOJI_CATEGORIES.map((cat) => (
              <div key={cat.name}>
                <h4 className="text-[11px] font-bold text-zinc-400 mb-1.5 uppercase tracking-wider">
                  {cat.name}
                </h4>
                <div className="grid grid-cols-8 gap-1.5">
                  {cat.emojis.map((emoji, idx) => (
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center text-xl hover:bg-white/10 rounded-lg transition active:scale-90 cursor-pointer"
                    >
                      {emoji}
                    </button>
                  ))}
      ...
      <div className="px-3 py-2 bg-[#111b21] border-t border-[#202c33] flex items-center justify-between shrink-0">
        ...
        <button
          ...
          className="p-1.5 rounded-xl bg-[#202c33] hover:bg-[#2a3942] text-[#00a884] hover:text-emerald-300 transition cursor-pointer shrink-0 ml-1"
