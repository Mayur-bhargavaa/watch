...
export const AddFriendModal: React.FC<AddFriendModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-2xl overflow-hidden p-6 sm:p-7">
        {/* Header */}
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-500/10 via-rose-500/5 to-purple-500/10 border border-amber-500/20">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-zinc-300 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Your Friend Code
              </span>
              <span className="text-[11px] text-slate-400 dark:text-zinc-400">Share with friends</span>
            </div>
            <div className="flex items-center justify-between bg-white dark:bg-zinc-950/80 border border-slate-200 dark:border-zinc-800 rounded-xl px-4 py-2.5">
              <span className="font-mono text-base font-black tracking-widest text-rose-600 dark:text-rose-400">
                {myFriendCode || 'LOADING...'}
              </span>
              <button
                onClick={handleCopyCode}
                className="flex items-center space-x-1.5 text-xs font-bold px-3 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white shadow-sm transition"
              >
                {copied ? (
                  <>
          ...
          <form onSubmit={handleAddFriend} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl font-mono text-sm uppercase tracking-wider text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-rose-500/50"
                  maxLength={16}
                />
              </div>
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 py-3 px-4 rounded-xl bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white font-bold text-sm shadow-lg shadow-rose-600/25 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
          ...
          {/* Explanation Tip */}
          <div className="text-[11px] text-slate-500 dark:text-zinc-400 leading-relaxed bg-slate-50 dark:bg-zinc-950/50 p-3.5 rounded-xl border border-slate-100 dark:border-zinc-800/60">
