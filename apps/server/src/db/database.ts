...
export class DatabaseService {
  ...
  updateDirectChatMessageStatus(messageId: string, status: string): void {
  ...
  markDirectMessagesAsRead(conversationId: string, readerUserId: string): { id: string; senderId: string }[] {
    ...
    if (otherId && otherId !== readerUserId) {
      ...
      `).all(conversationId, readerUserId) as any[];
  ...
  generateUniquePartnerCode(displayName?: string): string {
  ...
  ensureUserPartnerCode(
    ...
    const row = this.db.prepare('SELECT partner_code, display_name FROM users WHERE id = ?').get(userId) as any;
  ...
  getUserByPartnerCode(partnerCode: string): User | null {
    const stmt = this.db.prepare('SELECT * FROM users WHERE partner_code = ? COLLATE NOCASE');
    const row = stmt.get(partnerCode.trim().toUpperCase()) as any;
    if (!row) return null;
    return {
      id: row.id,
      email: row.email,
  ...
  getPartnerCodeStatus(userId: string): {
    currentCode: string;
    changesUsed: number;
    ...
    const userRow = this.db.prepare('SELECT partner_code FROM users WHERE id = ?').get(userId) as any;
    ...
    `).all(userId, ninetyDaysAgo) as { changed_at: string }[];
  ...
  changePartnerCode(userId: string, requestedCode: string): {
    success: boolean;
    error?: string;
    partnerCode?: string;
    changesRemaining?: number;
    ...
    const existing = this.db.prepare(
      'SELECT id FROM users WHERE partner_code = ? COLLATE NOCASE AND id != ?'
    ).get(cleanCode, userId) as any;
    if (existing) {
  ...
  getPartner(userId: string): PartnerConnection | null {
    const stmt = this.db.prepare(`
  ...
  connectPartner(userId: string, targetPartnerCode: string): PartnerConnection {
  ...
  disconnectPartner(userId: string): void {
    const stmt = this.db.prepare(`
      DELETE FROM partner_connections
  ...
  private getCanonicalUserPair(userIdA: string, userIdB: string): [string, string] {
  ...
  recordSessionBetweenUsers(userIdA: string, userIdB: string, minutes: number = 1): { streak: any; status: 'ALREADY_COMPLETED' | 'EXTENDED' | 'RESET_STARTED' } {
  ...
  getGameRoomByCode(code: string): GameRoom | null {
    const row = this.db.prepare('SELECT * FROM game_rooms WHERE room_code = ? COLLATE NOCASE').get(code.trim().toUpperCase()) as any;
    if (!row) return null;
    const players = this.getGameRoomPlayers(row.id);
    let gameState: any = null;
    if (row.game_state) {
      try { gameState = JSON.parse(row.game_state); } catch {}
    }
    return {
      id: row.id,
  ...
  getGameRoomById(id: string): GameRoom | null {
    const row = this.db.prepare('SELECT * FROM game_rooms WHERE id = ?').get(id) as any;
    if (!row) return null;
    const players = this.getGameRoomPlayers(row.id);
    let gameState: any = null;
    if (row.game_state) {
  ...
  findOpenWaitingGameRoom(gameType: string, maxPlayers: number, excludeUserId?: string): GameRoom | null {
    const rows = this.db.prepare(`
      ...
      ORDER BY created_at ASC
    `).all(gameType, maxPlayers) as any[];
  ...
  createPlan(plan: any): any {
    ...
    stmt.run(
      plan.id,
      plan.hostId || (plan.participants && plan.participants[0]?.userId) || 'u1',
      plan.title,
      plan.emoji || '✨',
      plan.type || 'custom',
      plan.date,
      plan.dateFormatted || plan.date,
      plan.time,
      plan.endTime || '',
      plan.timezone || 'IST',
      plan.description || '',
      JSON.stringify(plan.activities || []),
      JSON.stringify(plan.participants || []),
      plan.voting ? JSON.stringify(plan.voting) : null,
      plan.reminder || '30m',
      plan.recurring || 'none',
      JSON.stringify(plan.chatMessages || []),
  ...
  getPlans(): any[] {
    const rows = this.db.prepare('SELECT * FROM plans ORDER BY created_at DESC').all() as any[];
  ...
  updatePlan(id: string, updates: any): any | null {
    ...
    stmt.run(
      ...
      merged.type,
      merged.date,
      merged.dateFormatted,
      merged.time,
      merged.endTime || '',
      merged.timezone,
      merged.description || '',
      JSON.stringify(merged.activities || []),
      JSON.stringify(merged.participants || []),
      merged.voting ? JSON.stringify(merged.voting) : null,
      merged.reminder || '30m',
      merged.recurring || 'none',
      JSON.stringify(merged.chatMessages || []),
      merged.isPast ? 1 : 0,
      id
    );
