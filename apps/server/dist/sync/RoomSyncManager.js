...
import { calculateAuthoritativePosition } from '@synccinema/common';
...
export class RoomSyncManager {
    ...
    activeScreenPresenters = new Map();
    ...
    handleConnection(socket, room, user) {
        ...
        const activeUserIds = new Set(Array.from(clientSet)
    ...
    handleDisconnection(client, room, status = 'DISCONNECTED') {
    ...
    handlePlaybackCommand(client, room, payload) {
    ...
    handleReactionSend(client, room, payload) {
        const now = Date.now();
    ...
    // --- 4. Chat & Moderation ---
    handleChatSend(client, room, payload) {
        const now = Date.now();
    ...
    handleChatDelete(client, room, payload) {
        if (client.role !== 'HOST' && client.role !== 'CO_HOST') {
    ...
    handlePresenceState(client, room, payload) {
        this.broadcastToRoom(room.id, {
    ...
    // --- 5. Media Change ---
    handleMediaChange(client, room, payload) {
        if (client.role !== 'HOST' && client.role !== 'CO_HOST') {
    ...
    // --- 6. Host Transfer ---
    handleHostTransfer(client, room, payload) {
        if (client.role !== 'HOST') {
    ...
    handleVoiceSignal(client, room, payload) {
    ...
    handleVoiceState(client, room, payload) {
        this.broadcastToRoom(room.id, {
            type: 'voice:state',
            roomId: room.id,
            senderId: client.userId,
    ...
    handleCameraState(client, room, payload) {
        this.broadcastToRoom(room.id, {
            type: 'camera:state',
            roomId: room.id,
            senderId: client.userId,
    ...
    handleScreenState(client, room, payload) {
    ...
    sendStateSnapshot(client, room) {
        ...
        const activeUserIds = new Set(Array.from(clientsInRoom || [])
            .filter((c) => c.socket.readyState === WebSocket.OPEN || c.socket.readyState === WebSocket.CONNECTING)
            .map((c) => c.userId));
    ...
    handleGameAction(client, room, payload) {
        ...
        if (payload && payload.action) {
            mongoLogger.logGameEvent(room.id, client.userId, payload.gameType || 'trivia', payload.action, payload);
        }
    }
