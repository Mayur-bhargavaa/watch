...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  chrome.runtime.sendMessage({ type: 'GET_EXTENSION_STATUS' }, (res) => {
    if (res && res.data) {
      if (platformEl) platformEl.innerText = res.data.lastDetectedPlatform || 'None';
      if (syncStatusEl) syncStatusEl.innerText = res.data.isSyncActive ? 'Active' : 'Inactive';
      if (toggleBtn) {
  ...
  if (toggleBtn) {
    toggleBtn.addEventListener('click', async () => {
      ...
      if (syncStatusEl) syncStatusEl.innerText = nextState ? 'Active' : 'Inactive';
